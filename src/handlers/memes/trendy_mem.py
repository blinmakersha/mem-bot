...
from src.buttons.help.getter import TRENDY_MEM
...
@memes_router.message(F.text == TRENDY_MEM)
async def get_trendy_mem(message: types.Message, state: FSMContext) -> None:
    try:
        data = await fetch_mem('/mem/trendy-mem')
    ...
    if mem_info := data:
        if 'message' not in data:
            ...
            await message.answer_photo(
                ...
                reply_markup=get_buttons(mem_info['likes'], mem_info['dislikes'], add=True),
            )
        else:
